...
export default async function AdminUsersPage({ params }: { params: Params }) {
  ...
  const usersRaw = await User.find(
    ownerId ? { parent_client_id: ownerId, role: "user", isAdmin: false } : { _id: { $in: [] } }
  )
    .sort({ createdAt: -1 })
    .lean();

  const users = JSON.parse(JSON.stringify(usersRaw)) as AdminUserRow[];
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <CreateUserButton
              ...
              hasCapacity={hasActivePlan && !hasReachedPlanLimit(users.length, seatLimit)}
